...
int numSquares;
GLuint cube_vao, bonus_vao, platform_vao, ibo, points_vbo, cube_colours_vbo, pyramid_colours_vbo, platcolors_vbo, modelID, projID, viewID, normals_vbo;
GLfloat* normals;
float rotAngle = 0.0f;
...
{
	for (int i = 0; i < intdiceCount; i += 3)
	{
		...
		normals[in0 * 3 + 0] = normal.x;
		normals[in0 * 3 + 1] = normal.y;
		normals[in0 * 3 + 2] = normal.z;

		normals[in1 * 3 + 0] = normal.x;
		normals[in1 * 3 + 1] = normal.y;
		normals[in1 * 3 + 2] = normal.z;

		normals[in2 * 3 + 0] = normal.x;
		normals[in2 * 3 + 1] = normal.y;
		normals[in2 * 3 + 2] = normal.z;
...
void initLights()
{
	...
	currentLightPos = glm::vec3(0, 0, 4);
...
{	
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(normals), normals, GL_STATIC_DRAW); // This will not work
...
void display(void)	
{
	...
	transformObject(1.0f, Y_AXIS, angle += 4.0f, glm::vec3(0.0f, 0.0f, 0.0f));
...
void CreateDivisions()
{
	...
	normals = new GLfloat[12 * (rows * cols)];
	int index = 0;
	float x = 0;
	float y = 0;
	...
	CalculateNormals(plane_indicies, 12 * rows * cols, plane_vertices, 12 * rows * cols, 3, normals);
}
//---------------------------------------------------------------------
//
// main
